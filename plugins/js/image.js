...
(function (factory) {
  ...
  $.extend($.FE.DEFAULTS, {
    ...
    imageEditButtons: ['imageReplace', 'imageAlign', 'imageRemove', '|', 'imageLink', 'linkOpen', 'linkEdit', 'linkRemove', '-', 'imageDisplay', 'imageAlt', 'imageSize', 'imageCaption'],
    imageAltButtons: ['imageBack', '|'],
    imageCaptionButtons: ['imageBack', '|'],
    imageSizeButtons: ['imageBack', '|'],
    imageUploadURL: '/upload-post-image',
  ...
  $.FE.PLUGINS.image = function (editor) {
    ...
    function _addImage (link, data, loadCallback) {
      ...
      var $img = $('<img class="' + (editor.opts.imageDefaultDisplay ? 'fr-di' + editor.opts.imageDefaultDisplay[0] : '') + (editor.opts.imageDefaultAlign ? (editor.opts.imageDefaultAlign != 'center' ? ' fr-fi' + editor.opts.imageDefaultAlign[0] : '') : '') + '" src="' + link + '"' + data_str + (width ? ' style="width: ' + width + ';"' : '') + '><p class="clearfix"></p>');
    ...
    function _initInsertPopup (delayed) {
      ...
      if (uploadIndex >= 0) {
        ...
        upload_layer = '<div class="fr-image-upload-layer' + active + ' fr-layer" id="fr-image-upload-layer-' + editor.id + '"><strong>' + editor.language.translate('Drop image') + '</strong>(' + editor.language.translate('or click') + ')<div class="fr-form"><input type="file" accept="image/' + editor.opts.imageAllowedTypes.join(', image/').toLowerCase() + '" tabIndex="-1"></div></div>'
      }
...
/************************* CUSTOM CODE STARTS*********************************************/
    ...
    function _refreshCaptionPopup () {
      if ($current_image) {
        ...
        $popup.find('input').val($current_image.attr('title') || '').trigger('change');
      }
    }
    ...
    function _initCaptionPopup (delayed) {
      ...
      // Image by caption layer.
      var caption_layer = '';
      caption_layer = '<div class="fr-image-caption-layer fr-layer fr-active" id="fr-image-caption-layer-' + editor.id + '"><div class="fr-input-line"><input type="text" placeholder="' + editor.language.translate('Caption Text') + '" tabIndex="1"></div><div class="fr-action-buttons"><button type="button" class="fr-command fr-submit" data-cmd="imageSetCaption" tabIndex="2">' + editor.language.translate('Update') + '</button></div></div>'
      ...
      if (editor.$wp) {
        editor.events.$on(editor.$wp, 'scroll.image-caption', function () {
    ...
    function setCaption (caption) { 
      if ($current_image) {               
        var $popup = editor.popups.get('image.caption');
        $current_image.attr('title', caption || $popup.find('input').val() || '');
        var caption_width = $current_image.attr('style');
        caption_width = caption_width.split(':');  
        // alert($current_image.attr('style')+'   '+caption_width[1]);      
        if($current_image.attr('title') != ''){
          if($($current_image).parent().hasClass('thumbnail') === true){            
            $($current_image).next('figcaption').html($current_image.attr('title'));
          }else{            
            $( $current_image ).wrap( "<div class='post-caption-container'><figure class='thumbnail clearfix fr-fin fr-dib'></figure></div>" );
            $($current_image).after('<figcaption contenteditable="false" class="caption pull-center editor-img-caption">'+$current_image.attr('title')+'</figcaption>');  
          }
        }
        $popup.find('input:focus').blur();
...
/************************* CUSTOM CODE ENDS*********************************************/
        ...
        if (editor.events.trigger('image.beforeRemove', [$img]) !== false) {
          ...
          if ($img.get(0) == editor.$el.get(0)) {
            ...
            if ($img.get(0).parentNode.tagName == 'A') { 
              ...
              //$img.remove();
              // custom changes for caption removal
              if($($img).parent().parent().hasClass('post-caption-container') === true){
  ...
  $.FE.RegisterCommand('imageCaption', {
    ...
    title: 'Caption Text',
    callback: function () {
      this.image.showCaptionPopup();
    }
  });

  $.FE.RegisterCommand('imageSetCaption', {
    undo: true,
    focus: false,
    title: 'Update',
    refreshAfterCallback: false,
